...
const sendOrder2sellers = async (items, parentOrderId, UID) => {
  ...
  for (const SID in sellersProducts) {
    ...
    if (!findSeller) {
      findSeller = await adminModel.findById(SID);
    }
...
export const createOrder = async (req, res) => {
  ...
  if (userCart.promoCode) {
    ...
    const promo = await findPromoByCode(
      { body: { code: userCart.promoCode } },
      res
    );
  ...
  const stateList = await sendOrder2sellers(
    userCart.items,
    savedOrder._id,
    userData.id
  );
  ...
  res.json({
    msg: "Order created, and the orders sent to the sellers successfully",
    order: savedOrder,
  });
};
...
function updateOrderStatus(
  ...
  customerOrder.stateList.forEach((e) => {
    if (e[userData.id]) {
      e[userData.id] = newStatus;
    }
  });
...
export const updateDeliverStatus = async (req, res) => {
  ...
  if (!sellerData) {
    sellerData = await adminModel.findById(userData.id);
  } // check it on admin profile
  if (!sellerData) {
    res.json({
      msg: "updateDeliverStatus: user is not a seller. check the token",
    });
    return;
  } // raise error

  // get seller order
  let sellerOrder = sellerData.orders.find((o) =>
    o.parentOrderId.equals(orderId)
  );

  // get customer order
  const customerOrder = await orderModel.findById(orderId);
  if (!customerOrder) {
    res.json({ msg: "order not exist. check id" });
    return;
  }

  // get seller element in customerOrder for the seller
  var lastSellerOrderStatus = customerOrder.stateList.find(
    (obj) => obj[userData.id]
  )?.[userData.id];
  ...
  if (newStatus == "Cancelled") {
    ...
    if (lastSellerOrderStatus == "Processing") {
      ...
      for (const p of sellerOrder.products) {
        ...
        if (!insertedProduct) {
          res.json({
            msg: "updateDeliverStatus: product not exist when trying to increase the stock",
          });
          return;
        }
  ...
  //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    if (["Shipped", "Delivered"].includes(lastSellerOrderStatus)) {
      res.json({
        msg: "TERMINATED: order now in higher stage. can't return to processing",
      });
      //return;
    }
    ...
    if (lastSellerOrderStatus == "Processing") {
      res.json({ msg: "already Processing" });
    }
  }
  ...
  //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    if (lastSellerOrderStatus == "Delivered") {
      res.json({ msg: "already Delivered" });
    }
  } else {
